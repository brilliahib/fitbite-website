...
import {
  communityChatSchema,
  CommunityChatType,
} from "@/validators/community/community-chat-validator";
import { useCreateChatCommunity } from "@/http/community/create-message-community";

export default function MessageInputCommunity() {
  const form = useForm<CommunityChatType>({
    resolver: zodResolver(communityChatSchema),
    defaultValues: {
      image: undefined,
      message: "",
    },
  ...
  const { mutate: addHDHandler, isPending } = useCreateChatCommunity({
    onError: () => {
  ...
  const onSubmit = (body: CommunityChatType) => {
    addHDHandler(body);
  };
